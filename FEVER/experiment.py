...
def main():
    """
    Running ranking effectiveness experiment on FEVER
    """
    if not pt.started():
        pt.init()
    ...
    convex_mm = FFMinMaxInterpolate(alpha=0.1)
    convex_z = FFZScoreInterpolate(alpha=0.1)
    reciprocal = ReciprocalInterpolate(alpha=[1, 100])
    condorcet = CondorcetFuseInterpolate(alpha=0.2)
...
def output_to_file(res):
    ...
    """
    pd.concat(res).to_csv("FEVER_experiment.csv", index=False)
