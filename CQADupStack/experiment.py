...
def main():
    """
    Running ranking effectiveness experiment on CQADupStack English
    """
    if not pt.started():
        pt.init()
    ...
    index_path = "ffindex_cqadupstack_english_tct.h5"
    ...
    reciprocal = ReciprocalInterpolate(alpha=[1, 1])
...
def output_to_file(res):
    ...
    """
    pd.concat(res).to_csv("CQADupStack-English_experiment.csv", index=False)
