...
def main():
    """
    Running latency experiment on Arguana
    """
    if not pt.started():
        pt.init()

    cur_dir = os.getcwd()
    new_file_path = os.path.join(cur_dir, 'sparse_index_arguana/data.properties')
...
def output_to_file(res):
    ...
    """
    res.to_csv("Arguana_latency_experiment.csv", index=False)
