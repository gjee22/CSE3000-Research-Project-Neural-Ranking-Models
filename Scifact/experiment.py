...
def main():
    """
    Running ranking effectiveness experiment on Scifact
    """
    if not pt.started():
        pt.init()
    ...
    index_path = "ffindex_scifact_tct.h5"
    ...
    convex_z = FFZScoreInterpolate(alpha=0.3)
    reciprocal = ReciprocalInterpolate(alpha=[60, 60])
    condorcet = CondorcetFuseInterpolate(alpha=0.1)
...
def output_to_file(res):
    ...
    """
    pd.concat(res).to_csv("Scifact_experiment.csv", index=False)
